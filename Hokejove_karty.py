...
draw.text((325 * scale_factor, 830 * scale_factor), f"Poslední aktualizace: {Aktualizace}", fill="white", font=font_statistic)
...
for category, stats in stats_data.items():
    ...
    category_scores_text = f"{category_scores[category]}"
    text_bbox = draw.textbbox((0, 0), category_scores_text, font=font_value_bold)

    # Vypočet šířky a výšky textu
    category_scores_text_width = text_bbox[2] - text_bbox[0]
    category_scores_text_height = text_bbox[3] - text_bbox[1]
...
# Předpokládá se, že logo je uloženo ve složce 'TELH_Logos' uvnitř adresáře 'hokejove_karty'
logo_path = '/mount/src/hokejove_karty/TELH_Logos'
